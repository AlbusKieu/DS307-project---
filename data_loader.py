...
class InfluenceDataSet(Dataset):
    def __init__(self, file_dir, embedding_dim, seed, shuffle, model):
        self.graphs = np.load(os.path.join(file_dir, "Dataset/digg/digg/adjacency_matrix.npy")).astype(np.float32)
        ...
        self.influence_features = np.load(
                os.path.join(file_dir, "Dataset/digg/digg/influence_feature.npy")).astype(np.float32)
        logger.info("influence features loaded!")

        self.labels = np.load(os.path.join(file_dir, "Dataset/digg/digg/label.npy"))
        logger.info("labels loaded!")

        self.vertices = np.load(os.path.join(file_dir, "Dataset/digg/digg/vertex_id.npy"))
        ...
        vertex_features = np.load(os.path.join(file_dir, "Dataset/digg/digg/vertex_feature.npy"))
        ...
        embedding_path = os.path.join(file_dir, "Dataset/digg/digg/deepwalk.emb_%d" % embedding_dim)
